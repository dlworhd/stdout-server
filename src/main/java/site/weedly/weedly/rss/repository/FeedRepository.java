...
import site.weedly.weedly.rss.entity.Feed;
...
public interface FeedRepository extends JpaRepository<Feed, Long> {
	...
	@Cacheable(value="feeds", key = "'all' + #pageable.pageNumber + '-' + #pageable.pageSize")
	@Query("" +
			"SELECT new site.weedly.weedly.rss.dto.FeedResponse(f.id, c.channelName, c.channelLink, c.channelIcon, f.title, f.thumbnail, f.description, f.link, f.pubDate) FROM Feed f " +
			"LEFT JOIN f.channel c " +
			"ORDER BY f.pubDate DESC"
	)
	Page<FeedResponse> readAll(Pageable pageable);

	@Cacheable(value="feeds", key = "'International' + '-' + #pageable.pageNumber + '-' + #pageable.pageSize")
	@Query("" +
			"SELECT new site.weedly.weedly.rss.dto.FeedResponse(f.id, c.channelName, c.channelLink, c.channelIcon, f.title, f.thumbnail, f.description, f.link, f.pubDate) FROM Feed f " +
			"LEFT JOIN f.channel c " +
	...
	@Cacheable(value="feeds", key = "'Domestic' + '-' + #pageable.pageNumber + '-' + #pageable.pageSize")
	@Query("" +
			"SELECT new site.weedly.weedly.rss.dto.FeedResponse(f.id, c.channelName, c.channelLink, c.channelIcon, f.title, f.thumbnail, f.description, f.link, f.pubDate) FROM Feed f " +
			"LEFT JOIN f.channel c " +
